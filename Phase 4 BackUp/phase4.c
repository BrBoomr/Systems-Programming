...
void Add_I_Item(char *, int, int, char*, int, int, char *);					//Reads in from the source file and sends the broken-up parameters as I_Items
...
//-OBJECT CODE PROTOTYPES---------------------------------------------------------------------------------------------------------------
void Create_Obj_Table();		//Initializes the struct Array for the Object Table
void CreateObjectCode();		//Processes the OpCode and Symbol Addresses into Machine Code
void Add_Obj_Item(char *, char * , int);	//Adds an object to the object table (for referencing)
int getSymbolAddress(int, char*);			//Retrieves the Operands Address (by searching the Symbol Table)
int getOpCodeAddress(char*);				//Retrieves the appropriate LOCCTR by looking for the I_Table index that contains the matching machine code
												//This was done by adding the Machine Code to the I_Table property M_Code during Pass Two.
void pToObjectFile();
//---------------------------------------------------------------------------------------------------------------------------------------

//-LISTING FILE PROTOTYPES---------------------------------------------------------------------------------------------------------------
...
void LoadProcedure(char*);
void DumpProcedure(int, int);
...
void dump(char *_start, char *_end)
{
...
void assemble(char *filename) {
	...
	if (file == NULL) {
		...
		do {
			...
			if (_source[0] == '.') {
				Add_I_Item(_source, LOCCTR, Search_O_Table(_mnemonic), _operand, _line, _code, _comment);
			}
			else{
		...
		while (fgets(_source, 100, file)) {
			...
			if (cmp != 0) {
				...
				if (_source[0] == '.') {
					...
					Add_I_Item(_source, LOCCTR, _OpCode, _operand, _line, _code, _comment);
					LOCCTR += LOC_INCR;
				}
			}
...
void Add_I_Item(char *Source, int Location, int OpCode, char *Operand, int Line, int Error, char *Comment) {
	int index = 0;
	if (Line < T_SIZE) {
		index = Line - 1;
...
void Create_O_Table() {			//Initiliaze the O_Table[] Array to = '0'. Will be used as a condition to where the "top" of the array is
	for (int x = 0; x < T_SIZE; x++) {
		...
		memset(&O_Table[x]->Mnemonic[0], '\0', sizeof(O_Table[x]->Mnemonic));
		O_Table[x]->OpCode = 0;
	}

	//DO NOT DELETE. ADD ITEMS LIKE THIS.
	//strncpy(O_Table[10]->Mnemonic, "NEW", 10);
	//O_Table[10]->OpCode = 7;
}

void Add_O_Item(char *Mnemonic, int OpCode) {
	int index = 0;
	while (O_Table[index]->Mnemonic[0] != '\0') {
		index++;
	}
	strncpy(O_Table[index]->Mnemonic, Mnemonic, 10);
...
int Search_O_Table(char* Mnemonic) {
	int cmp = 0;
	int index = 0;
	...
	while (O_Table[index]->Mnemonic != '\0') {
		cmp = strcmp(O_Table[index]->Mnemonic, Mnemonic);
		if (cmp == 0) {
...
void Create_S_Table() {
	for (int x = 0; x < T_SIZE; x++) {
		...
		memset(&S_Table[x]->Label[0], '\0', sizeof(S_Table[x]->Label));
		S_Table[x]->address = 0;
	}
}

void Add_S_Item(char *Label, int address) {
	int index = 0;
	while (S_Table[index]->Label[0] != '\0') {
		index++;
	}
	strncpy(S_Table[index]->Label, Label, 10);
	S_Table[index]->address = address;
}


int Search_S_Table(char *Label) {
	int cmp = 0;
	int index = 0;
	int _found = 0;
	while (S_Table[index]->Label != '\0') {
		cmp = strcmp(S_Table[index]->Label, Label);
		if (cmp == 0) {
			_found = 1;
			goto S_Found;
		}
...
void pToSymFile() {
	...
	for (int x = 0; x < T_SIZE-1; x++) {
		fprintf(S_File, "%s\t\t %x\n", S_Table[x]->Label, S_Table[x]->address);
	}
	fclose(S_File);
}
//-----------------------------------------------------------------END PHASE 2
...
void CreateObjectCode() {
	...
	FILE *OBJECT_FILE = fopen("OBJECT_TABLE.txt", "w");
	for (int x = 0; x < T_SIZE; x++) {
		...
		//If BYTE
			...
			if (_operand[0] == 'X') {
				while (_operand[temp] != 39) {
					...
					strncpy(temp_string, _operand, sizeof(_operand));
					//convert the current character of the operand,  store it inside the temporary variable
					sprintf(temp_string, "%c", _operand[temp]);
					//concatenate to the object string
					strcat(_object, temp_string);
					temp++;
				}
			}
		...
		/*if (_object[0] == '\0' || _opcode == NULL_OPCODE) {
			continue;
		}*/
		Add_Obj_Item(_object, _address, _opcode);
	}
	for (int x = 0; x < T_SIZE; x++) {
		/*if (Obj_Table[x]->Object[0] == '\0' || Obj_Table[x]->OpCode == NULL_OPCODE) {
			continue;
		}*/
...
void Create_Obj_Table() {
	for (int x = 0; x < T_SIZE; x++) {
		...
		memset(&Obj_Table[x]->Object[0], '\0', sizeof(Obj_Table[x]->Object));
		memset(&Obj_Table[x]->Address[0], '\0', sizeof(Obj_Table[x]->Address));
		Obj_Table[x]->OpCode = 0;
		Obj_Table[x]->Used = 0;
		memset(&I_Table[x]->M_Code[0], '\0', sizeof(I_Table[x]->M_Code));
	}
}

void Add_Obj_Item(char *object, char *address, int opcode) {
	int index = 0;
	while (Obj_Table[index]->Used != 0) {
		index++;
	}
	strncpy(Obj_Table[index]->Object, object, 10);
	strncpy(Obj_Table[index]->Address, address, 5);
	Obj_Table[index]->OpCode = opcode;
	Obj_Table[index]->Used = 1;
	//Since we only ever generate the Machine Code until Pass 2, we give the Intermediate Table the value now.
	//It works nicely as any line that does not generate Machine Code will be left blank, a delimeter I can use. 
	strncpy(I_Table[index]->M_Code, object, 10);
}
...
int getSymbolAddress(int OpCode, char *Operand) {
	...
	if (cmp == 0) {
		...
		//We're adding 8000 to 1xxx due to the indexing. [0001] + [1000] = [1001] == 9(d)
		//or
		//8000(d) = 32768(h)
		address_increment = 32768;
	}

	while (S_Table[index]->Label != '\0' && index<50) {
		cmp = strcmp(S_Table[index]->Label, Operand);
		if (cmp == 0) {
...
int getOpCodeAddress(char *object) {
	int cmp;
	int x = 0;
	while (strcmp(I_Table[x]->M_Code, object) != 0) {
		x++;
	}
	return I_Table[x]->Location;
}

void pToObjectFile() {
	FILE *Obj_File = fopen("Object.txt", "w");
	char H_RECORD[20]; H_RECORD[0] = '\0';
	char E_RECORD[20]; E_RECORD[0] = '\0';
	//Symbol Table contains starting address, and prog_length is a global variable declared before and assigned in assemble()
	sprintf(H_RECORD, "H%s  %06X%06X", S_Table[0]->Label, S_Table[0]->address, prog_length);
	sprintf(E_RECORD, "E%06X", S_Table[0]->address);
	fprintf(Obj_File,"%s\n", H_RECORD);
	...
	//printf("%s\n", T_ADDRESS);
	int index = 1;				//Our object code includes the Start directive, so we skip over it by moving to the next index
	int displace = 0;			//Used to displace the T_RECORD so that we can accomodate for another line that will not fit.
	while (Obj_Table[index]->OpCode != 0XF2){
		T_RECORD[0] = '\0';
		T_MCODE[0] = '\0';
		TSIZE[0] = '\0';
		T_ADDRESS[0] = '\0';
		//Keep incrementing the index until it finds a non-null machine code
		while (Obj_Table[index]->Object[0] == '\0') {
			index++;
		}
		//Retrieve the LOCCTR in which the Machine Code was found.
			//We can achieve this as the we assigned the object code to the M_CODE property in I_Table to it's respective index.
		sprintf(T_ADDRESS, "%06X", getOpCodeAddress(Obj_Table[index]->Object));
		while (strlen(T_MCODE) < 60) {
			/*if (displace == 1) {
				if (strlen(T_MCODE) >= 42) {
					break;
				}
			}*/
			if (I_Table[index]->OpCode == 0xF5 || I_Table[index]->OpCode == 0xF6) {
				break;
			}
			strcat(T_MCODE, Obj_Table[index]->Object);
			index++;
			//If the next index is the END directive, break out.
			if (Obj_Table[index]->OpCode == 0XF2) {
				break;
			}
			//If the length of the next index by the current T_RECORD is greater than 60, break.
			//This will create a new T_RECORD with the next corresponding index.
			if (strlen(Obj_Table[index]->Object) + strlen(T_MCODE) > 60) {
				break;
			}
		}
		displace++;
		//Since 60 bits is 30 bytes
		//We store it in local variable for easier conversion.
		int t_length = strlen(T_MCODE) / 2;
		//Convert from int to hexadecimal, then assign as string to TSIZE
		sprintf(TSIZE, "%02X", t_length);
		//Create the T_RECORD by concatenation using sprintf
		sprintf(T_RECORD, "T%s%s%s", T_ADDRESS, TSIZE, T_MCODE);
...
//LISTING FILE FUNCTIONS---------------------------------------------------------

void pToListingFile() {
	FILE *List_File = fopen("Listing.txt", "w");
	for (int x=1; x < T_SIZE; x++) {
		if (strcmp(Obj_Table[x]->Object, "FF0000") == 0) {
			continue;
		}
		if (I_Table[x]->Error == 0 || I_Table[x]->Error == 4) {
			fprintf(List_File, "%X\t|%s\t\t\t|%s\n", I_Table[x]->Location, Obj_Table[x]->Object, I_Table[x]->Source);
		}
		else{
			Obj_Table[x]->Object[0] = '\0';
			fprintf(List_File, "%X\t|%s\t\t\t|%s\n", I_Table[x]->Location, Obj_Table[x]->Object, I_Table[x]->Source);
			if (I_Table[x]->Error == 1) {
				fprintf(List_File,"\tE1: Duplicate Label found in SymTable\n");
			}
			else if (I_Table[x]->Error == 2) {
				fprintf(List_File,"\tE2: Error in OpCode(check spelling)\n");
			}
			else if (I_Table[x]->Error == 3) {
				fprintf(List_File,"\tE3: Critical Error.\n");
			}
		}
	}
	fclose(List_File);
...
//Global variable to store Executable Address from E-Record
unsigned int e_address;

void LoadProcedure(char *filename) {
	char _source[100];
	char _fname[10];
	
	unsigned int _addr, _size, _obj;
	BYTE value;
	printf("File Chosen: %s\n", filename);
	FILE *file = fopen(filename, "r");
	printf("-----------------\n");
	if (file == NULL) {
		...
		fgets(_source, 100, file);
		sscanf(_source + 1, "%s", _fname);
		int x = 0;
		while (_source[0] != 'E') {
			int position = 9;
			fgets(_source, 100, file);
			if (_source[0] == 'E') {
				sscanf(_source + 1, "%x", &e_address);
				break;
			}
			sscanf(_source + 1, "%6X%2X%X", &_addr, &_size, &_obj);
			//printf("%s\n", _source);
			//printf("%x\n", _addr);
			//printf("SIZE: %x\n", _size);
			//printf("%x\n\n", _obj);
			for (int y = 0; y < (int)_size*2; y+=2) {
				_obj = 0;
				sscanf(_source + (position+y), "%02X", &_obj);
				value = (BYTE)_obj;
				PutMem(_addr++, &value, 0);
				//printf("%02X | %02X || %d\n", _obj, value, y);
			}
			x++;
		}
	...
	printf("Program Executable Address: %X\n", e_address);
}

void DumpProcedure(int _start, int _end) {
...
void ExecuteProcedure() {
	ADDRESS _addr = (ADDRESS)e_address;
	SICRun(&_addr, 0);
}
