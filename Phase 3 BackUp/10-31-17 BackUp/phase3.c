...
void readIntermData(char *, int, int, char*, int, int);					//Reads in from the source file and sends the broken-up parameters as I_Items
...
void assemble(char *filename) {
	...
	if (file == NULL) {
		...
		//------------------------------------------------------------------------------------------
		char _stm[100];		//Holds the original source line to be broken up to the next variables.
		...
		int temp;
		int _location;			//Used to add upon the locCtr
		int _byte;
		//------------------------------------------------------------------------------------------
		//STEPS:
		...
		do {
			...
			if (_stm[0] == '.') {
				...
				if (cmp == 0) {
					...
					d_locCtr++;
					_code = 4;
					readIntermData(_stm, locCtr, Search_H_Table(_p2), _p3, _line, _code);
				}
				else if (d_locCtr == 0) {
					locCtr = startAddr = 0;
					d_locCtr++;
			...
			if (cmp != 0) {
				...
				//Statement field is empty, therefore p1 and p2 are evaluated.
					...
					if (Search_H_Table(_p1) != NULL_OPCODE) {
						...
						//BYTE
						//Byte related-operations will be using special conditions such as (_byte & 1)
							...
							if (_p2[0] == 'C') {
								...
								temp = 2;
								//ASCII 39 == ' \' '
								//We could do the aforementioned, but an number hurts less to use.
								while (_p2[temp] != 39) {
									_byte++;
									temp++;
								}
					...
					//If p1 is not found within the H_Table, set the error code to 2 and continue with reading the data.
					else {
						_code = 2;
					}
					readIntermData(_stm, locCtr, Search_H_Table(_p1), _p2, _line, _code);
					locCtr += _location;
				}
				//[3]
				//If the first statement is not empty, then evaluate p1 p2 and p3
				else {
					...
					if (Search_H_Table(_p2) != NULL_OPCODE) {
						...
						//BYTE
						//Special operations occur here. We must use bitwise algorithms (_byte&1) (>>)
						else if (_OpCode == 0xF3) {
							//Same as before.
							if (_p3[0] == 'C') {
								temp = 2;
				...
				_code = 4;
				int _OpCode = Search_H_Table(_p1);
				readIntermData(_stm, locCtr, _OpCode, _p2, _line, _code);
				goto P1_Exit;
			}
		}
...
void Show_I_Table() {
	for (int x = 0; x < T_SIZE; x++) {
		printf("-------------------------\n");
		printf("Location: %d\t Opcode: %#x\t Operand: %s\t Line: %d\t Error: %d\n)", I_Table[x]->Location, I_Table[x]->OpCode, I_Table[x]->Operand, I_Table[x]->Error, I_Table[x]->Line);
	}
}

void readIntermData(char *Source, int Location, int OpCode, char *Operand, int Line, int Error) {
	int index = 0;
	...
	I_Table[index]->Line = index;
	I_Table[index]->Error = Error;
}

void pToIntermFile() {
	FILE *I_File = fopen("intermediate.txt", "w");
	fprintf(I_File,"ORIGINAL SOURCE CODE\n");
	for (int x = 0; x < T_SIZE; x++) {
		fprintf(I_File, "%s\n", I_Table[x]->Source);
	}
	fprintf(I_File, "----------------------------------------------------------------------------------------------------------\n");
	fprintf(I_File, "Line\t |\t OpCode\t |\t Operand\t |\t\t Location\t |\t Error\t |\n");
	for (int x = 0; x < T_SIZE; x++) {
		...
		fprintf(I_File, "%d\t\t %#x\t\t %-8s\t\t\t %x\t\t\t %d\n", I_Table[x]->Line, I_Table[x]->OpCode, I_Table[x]->Operand, I_Table[x]->Location, I_Table[x]->Error);
	}
	fprintf(I_File, "\n Codes:\n");
	fprintf(I_File, "----------------------------\n");
