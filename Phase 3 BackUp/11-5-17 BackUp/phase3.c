...
//------------------------------------------------------------------------------------------------
void Show_O_Table();			//TESTING PURPOSES ONLY. DELETE LATER.->Shows the Table Hash Table in it's entirity.
	...
	if (file == NULL) {
		...
		while (fgets(_source, 100, file)) {
			...
			//If "END" directive is found
				...
				_code = 4;
				int _OpCode = Search_O_Table(_label);
				readIntermData(_source, LOCCTR, _OpCode, _operand, _line, _code, _comment);
				goto P1_Exit;
			}
		}
...
void Show_O_Table() {
	for (int x = 0; x < T_SIZE; x++) {
		printf("------------------------\n");
		printf("\tINDEX: %d\n", x);
		printf("Stm: %-10s\t OpCode: %#x\n",O_Table[x]->stm,O_Table[x]->OpCode);
	}
}
