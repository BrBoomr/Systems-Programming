...
void Add_Obj_Item(char *, char * , int);
...
void pToIntermFile() {
	...
	for (int x = 0; x < T_SIZE; x++) {
		if (I_Table[x]->OpCode != NULL_OPCODE) {
...
void CreateObjectCode() {
	...
	FILE *OBJECT_FILE = fopen("ObjTable.txt", "w");
	for (int x = 0; x < T_SIZE; x++) {
...
void pToObjectFile() {
	...
	sprintf(H_RECORD, "H%s_%06X%06X", S_Table[0]->stm, S_Table[0]->address, prog_length);
